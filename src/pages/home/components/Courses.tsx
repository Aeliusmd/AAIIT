      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {courses.map((course) => (
            <div
  key={course.id}
  className="bg-white rounded-[28px] overflow-hidden group hover:-translate-y-1 transition-all duration-300 cursor-pointer border border-gray-100 hover:border-blue-200"
  onClick={() =>
    setActiveTab(activeTab === course.id ? null : course.id)
  }
>
  {/* Image */}
  <div className="relative h-52 overflow-hidden">
    ...
    {/* Top Tag */}
    <div
      className={`absolute top-4 left-4 px-3 py-1 rounded-full text-xs font-semibold ${course.tagColor}`}
    >
      {course.tag}
    </div>
    ...
    {/* Icon */}
    <div
      className={`absolute bottom-4 right-4 w-10 h-10 flex items-center justify-center rounded-xl ${course.iconBg}`}
    >
  ...
  <div className="p-7">
    ...
    <div className="mb-4">
      <span
        className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold ${course.statusColor}`}
      >
    ...
    {course.register && (
      <button
        ...
        className="w-full mb-5 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center gap-2"
      >
        Register Now
    ...
    <div className="flex items-center justify-between pt-4 border-t border-gray-100">
      ...
      >
        <span>
          {activeTab === course.id ? "Less" : "Details"}
        </span>
